...
def submit6():
    ...
    def a6():
        ...
        cursor = mydb.cursor()
        cursor.execute(
            "insert into fee (fee_month,fee_status,SID)values('"
            + feemonth
            + "','"
            + fstatus
            + "','"
    ...
    submitfee1 = Button(
        fee, text="SUBMIT", font=("italic", 15), bg="#10044d", fg="white", command=a6
    )
...
def submit4():
    ...
    update1 = Button(
        update, text="UPDATE", font=("italic", 15), bg="#10044d", fg="white", command=a4
    )
...
def submit2():
    ...
    insert1 = Button(
        insert, text="INSERT", font=("italic", 15), bg="#10044d", fg="white", command=a1
    )
...
def submit3():
    ...
    dele1 = Button(
        dele, text="DELETE", font=("italic", 15), bg="#5cf5f7", fg="#10044d", command=a2
    )
...
def Hostellogin():
    ...
    ide = Label(
        hostel, text="HOSTEL LOGIN PAGE", bg="#041d78", fg="#83e6e6", font=("bold", 20)
    )
    ...
    def submit():
        ...
        if name == "" or password == "":
            ...
            sql = (
                "SELECT * FROM hostellogin WHERE  name = '%s' AND  password = '%s'"
                % (e1.get(), e2.get())
            )
...
def logout():
    ...
    def logoutsubmit():
        ...
        if (
            ...
            cursor = con.cursor()
            cursor.execute(
                "insert into logout (hostelids,sname,place,sem,date,time)  values('"
                + hostelids
                + "','"
                + sname
                + "','"
    ...
    logoutsubmit = Button(
        logout, text="SUBMIT", font=("italic", 15), bg="#0afff7", command=logoutsubmit
    )
...
def LOG():
    ...
    butLO = Button(
        log, text="LOGOUT", font=("italic", 20), bg="#07f7cb", command=logout
    )
